...
dataset %<>%
  ...
  mutate(
    correct_a = equals(flashes, antwoord_flash_a),
...
beep_1_2 %>%
  .[[1]] %>%
  .[1] %>%
  write.xlsx("antwoord_flash_ab_per_sociale_conditie_beep_1.xlsx", showNA = FALSE)
beep_1_2 %>%
  .[[2]] %>%
  .[1] %>%
  write.xlsx("antwoord_flash_ab_per_sociale_conditie_beep_2.xlsx", showNA = FALSE)
...
dataset <- haven::read_sav("~/15 jun 22/dataset.sav")

cat("gemiddelde antwoord_flash_ab per beep en sociale conditie\n")
dataset %>%
  filter(participant != 1) %>%
  group_by(beep, sociale_condities) %>%
  select(antwoord_flash_ab) %>%
  summarise(mean(antwoord_flash_ab))

cat("")
dataset %>%
  filter(participant != 1) %>%
  stats::aov(antwoord_flash_ab ~ as.factor(sociale_condities) + as.factor(beep), data = .) %>%
  supernova::pairwise(correction = "Bonferroni", alpha = 0.05, plot = FALSE)

dataset %>%
  filter(participant != 1) %>%
  group_by(beep) %>%
  group_map(~ stats::aov(antwoord_flash_ab ~ as.factor(sociale_condities), data = .))

cat("summary of Anova model statistics\n")
dataset %>%
  filter(participant != 1) %>%
  stats::aov(antwoord_flash_ab ~ as.factor(sociale_condities) + as.factor(beep), data = .) %>%
  summary()
...
cat("ANOVA within-Ss designs ('repeated measures')")
ez::ezANOVA(dataset,
  dv = "antwoord_flash_ab",
  wid = participant,
  within = .(sociale_condities),
  between = .(beep),
  detailed = TRUE,
  type = 1
)
...
dataset %<>%
  mutate(antwoord_flash_reactietijd_ab = coalesce(antwoord_flash_reactietijd_a, antwoord_flash_reactietijd_b))

dataset %>%
  filter(participant != 1) %>%
  stats::aov(formula = antwoord_flash_reactietijd_ab ~ as.factor(sociale_condities) + as.factor(beep), data = .) %>%
  supernova::pairwise(correction = "Bonferroni", alpha = 0.05, plot = FALSE)
...
dataset %<>%
  mutate(correct = coalesce(correct_a, correct_b))

cat("gemiddelde reactietijd naar (niet-)correcte antwoorden")
dataset %>%
  select(antwoord_flash_reactietijd_ab, correct) %>%
  drop_na() %>%
  group_by(correct) %>%
  group_map(~ c(.)) %>%
  map_depth(2, ~ mean(.))

cat("frequencies correct/niet-correcte antwoord")
dataset$correct %>% table()

c("odds per sociale condities")
dataset %>%
  select(sociale_condities, correct) %>%
  group_by(sociale_condities) %>%
  group_map(~ select(., correct) %>%
    unlist() %>%
    table() %>%
    {
      divide_by(.[2], .[1])
...
haven::write_sav(dataset, "~/18 jun 22/dataset.sav")
readr::write_rds(dataset, "~/18 jun 22/dataset.rds")
xlsx::write.xlsx(dataset, "~/18 jun 22/dataset.xlsx", showNA = FALSE)
