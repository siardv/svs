...
for (i in 1:length(ls_temp)) {
  ...
  if (length(ls_temp[[i]]) == 2) {
    dataset$antwoord_flash_reactietijd_a <<- ls_temp[[i]][[1]]
    dataset$antwoord_flash_reactietijd_b <<- ls_temp[[i]][[2]]
  } else {
    dataset$antwoord_flash_reactietijd_a <<- NA_complex_
    dataset$antwoord_flash_reactietijd_b <<- NA_complex_
  }
}
