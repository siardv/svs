...
dataset %>%
  group_by(participant, sociale_condities) %>%
  group_data() %>%
  list(dataset %>% group_by(participant, sociale_condities) %>% group_map(~ {
    colMeans(.[c(
      "antwoord_flash_reactietijd_a",
      "antwoord_flash_reactietijd_b",
      "antwoord_communicatie_reactietijd_a",
...
write.xlsx(df, "reactietijd_flash_comm.xlsx", showNA = FALSE)
